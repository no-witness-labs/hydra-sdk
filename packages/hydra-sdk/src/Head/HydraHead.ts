...
const awaitMessageWithSchema = <A, I>(
  ...
  Effect.gen(function* () {
    ...
    const socketMessages = yield* PubSub.subscribe(socketController.messageQueue);
  ...
  "HydraHeadController",
  {
    effect: Effect.gen(function* () {
      ...
      const initialize = Effect.gen(function* () {
        ...
        yield* Effect.log(`Initialization complete, status is now is ${[hydraStateMachine.getStatus()]}`);
      });
      ...
      const recover = (recoverTxId: string) => Effect.gen(function* () {
        ...
        const message: Protocol.RecoverMessage = { tag: "Recover", recoverTxId }
        yield* socketController.sendMessage(JSON.stringify(message))
        
        yield* Effect.log(`Awaiting CommitRecovered message`);
        
        const commitRecoveredMessage = yield* Effect.gen(function* () {
          var msg = yield* awaitMessageWithSchema(socketController, Protocol.CommitRecoveredMessageSchema, Duration.seconds(5))
          
          while (msg.recoveredTxId !== recoverTxId) {
            yield* Effect.log(`Received wrong recoveredTxId: ${msg.recoveredTxId}, waiting for ${recoverTxId}`);
            msg = yield* awaitMessageWithSchema(socketController, Protocol.CommitRecoveredMessageSchema, Duration.seconds(5));
          }
          
          return msg;
        }).pipe(Effect.timeout(Duration.minutes(1)));

        yield* Effect.log(`Recover action complete, recovered TxId: [${commitRecoveredMessage.recoveredTxId}]`);
      });
      ...
      const commit = Effect.gen(function* () {
        ...
        yield* Effect.log(`Commit complete, status is now is ${[hydraStateMachine.getStatus()]}`);
      });

      const newTx = (transaction: Protocol.TransactionMessage) => Effect.gen(function* () {
        ...
        yield* Effect.log(`Sending "NewTx" message with txId: ${transaction.txId}`);
        ...
        const txValidMessage = yield* Effect.gen(function* () {
          var msg = yield* awaitMessageWithSchema(socketController, Protocol.TxValidMessageSchema, Duration.seconds(5))
          
          while (msg.transactionId !== transaction.txId) {
            yield* Effect.log(`Received wrong recoveredTxId: ${msg.transactionId}, waiting for ${transaction.txId}`);
            msg = yield* awaitMessageWithSchema(socketController, Protocol.TxValidMessageSchema, Duration.seconds(5));
          }
          
          return msg;
        }).pipe(Effect.timeout(Duration.minutes(1)));

        yield* Effect.log(`NewTx complete, txValidMessage TxId is ${[txValidMessage.transactionId]}`);
      });

      const decommit = (decommitTx: Protocol.TransactionMessage) => Effect.gen(function* () {
        ...
        yield* Effect.log(`Sending "Decommit" message with txId: ${decommitTx.txId}`);
        const message: Protocol.DecommitMessage = { tag: "Decommit", decommitTx }
        yield* socketController.sendMessage(JSON.stringify(message))

        yield* Effect.log(`Awaiting CommitRecovered message`);
        
        const txValidMessage = yield* Effect.gen(function* () {
          var msg = yield* awaitMessageWithSchema(socketController, Protocol.DecommitApprovedMessageSchema, Duration.seconds(5))
          
          while (msg.decommitTxId !== decommitTx.txId) {
            yield* Effect.log(`Received wrong recoveredTxId: ${msg.decommitTxId}, waiting for ${decommitTx.txId}`);
            msg = yield* awaitMessageWithSchema(socketController, Protocol.DecommitApprovedMessageSchema, Duration.seconds(5));
          }
          
          return msg;
        }).pipe(Effect.timeout(Duration.minutes(1)));

        yield* Effect.log(`NewTx complete, txValidMessage TxId is ${[txValidMessage.decommitTxId]}`);
      });
      ...
      const close = Effect.gen(function* () {
        ...
        yield* Effect.log(`Close action is complete, status is now is ${[hydraStateMachine.getStatus()]}`);
      });
      ...
      const contest = Effect.gen(function* () {
        ...
        const headIsContestedMessage = yield* awaitMessageWithSchema(socketController, Protocol.HeadIsContestedMessageSchema, Duration.seconds(5))

        yield* Effect.log(`Contest action is complete, snapshotNumber now is ${[headIsContestedMessage.snapshotNumber]}`);
      });
      ...
      const fanout = Effect.gen(function* () {
        ...
        yield* Effect.log(`Awaiting [${status}] status`);
        hydraStateMachine.awaitStatus(status)
        yield* Effect.log(`Contest action is complete, status is now ${[hydraStateMachine.getStatus()]}`);
      });
