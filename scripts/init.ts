...
function getAllFiles(dir: string, files: Array<string> = []): Array<string> {
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      ...
      FILE_EXTENSIONS.some((ext) => entry.endsWith(ext)) ||
      INCLUDE_FILES.includes(entry)
    ) {
      files.push(fullPath);
    }
  }
...
async function main(): Promise<void> {
  ...
  const replacements: Record<string, string> = {
    projectName:
      args.projectName || (await prompt("Project name", "my-project")),
    projectScope:
      args.projectScope ||
      (await prompt("Package scope (include trailing /)", "@myorg/")),
    githubOrg: args.githubOrg || (await prompt("GitHub org/username", "myorg")),
    projectTitle:
      args.projectTitle || (await prompt("Project title", "My Project")),
    copyrightHolder:
      args.copyrightHolder || (await prompt("Copyright holder", "Your Name")),
    copyrightYear:
      args.copyrightYear || (await prompt("Copyright year", currentYear)),
  };
